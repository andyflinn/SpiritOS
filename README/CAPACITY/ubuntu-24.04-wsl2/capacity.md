...
| measured at | 2026-09-26T04:22:42.064Z |
| tree | `a95b0d1` |
...
| RAM, personal node | **124 MB** at rest |
| RAM, relay | **64 MB** at rest, before any connection |
| disc, the install | **4233 KB** in 134 files |

| fixed cost | RSS |
|---|---|
| bare `node`, nothing loaded | **42 MB** |
| a personal node at rest | **124 MB** |
...
| 200 | 83 MB | 19 MB | 98 KB |
| 400 | 94 MB | 30 MB | 76 KB |
| 800 | 103 MB | 39 MB | 49 KB |

**~23 KB per held stream in the process**, from the slope of the last segment (the 100→200 segment reads 71 KB, which is the spread to expect).
`STREAMS_PER_MB = 16` implies 64 KB, so the guess is 64% pessimistic.
...
| the two boxes | |
|---|---|
| relay, 100 MB RAM | **~1591 members connected at once** (36 MB headroom / 23 KB) |
