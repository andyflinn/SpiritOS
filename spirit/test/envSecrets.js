...
//
//   GROK_MANAGMENT_KEY — management-api.x.ai, GET only (2026-09-22)
...
if (sub('Bearer ${ENV:GROK_API_KEY}', 'evil.example', 'POST') === 'Bearer ${ENV:GROK_API_KEY}' &&
    ...
    sub('Bearer ${ENV:GROK_MANAGMENT_KEY}', 'api.x.ai', 'GET') === 'Bearer ${ENV:GROK_MANAGMENT_KEY}') {
...
if (sub('Bearer ${ENV:GROK_MANAGMENT_KEY}', 'management-api.x.ai', 'GET') === 'Bearer M-SECRET' &&
    sub('Bearer ${ENV:GROK_MANAGMENT_KEY}', 'management-api.x.ai', 'get') === 'Bearer M-SECRET') {
...
const writes = ['POST', 'PUT', 'PATCH', 'DELETE'].filter(function (m) {
  return sub('Bearer ${ENV:GROK_MANAGMENT_KEY}', 'management-api.x.ai', m) !== 'Bearer ${ENV:GROK_MANAGMENT_KEY}';
});
if (!writes.length) {
