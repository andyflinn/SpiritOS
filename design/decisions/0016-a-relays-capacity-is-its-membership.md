...
> constraints. and the sooner we do it, the lower the impact."*

**Low in code and rising in design, and the rise is the point.** Nothing
is built, so nothing has to be unbuilt today. But the cost is not flat
and it is not recoverable: every app written before the cap lands is
written assuming concurrency it will not have, and each one is a retrofit
that did not need to exist.

**It is a one-way door.** Once the cap is the guarantee, relaxing it
gives back the bound that makes a relay's memory a function of its
membership — the whole of this decision. So it will not be relaxed, which
means the constraint is permanent and the only variable is **how much has
been built on the other side of it** when it lands.

**What the constraint permanently costs, stated plainly:** the product is
sequential. No app may fan out and wait; no screen may assume several
answers arriving together; every operation touching more than one peer is
a queue with progress, not a batch with a spinner.

**And that is why it is affordable.** The constraint is not foreign to
this product — it matches what the shell already is:

> **Andy:** *"the UI approach only presents one app/dialog at the time.
> so how many requests can be generated by normal use anyways....."*

A constraint consonant with a product's existing character costs far less
to adopt permanently than one imposed against it. The proof from
2026-09-20 says the same thing from the other end: **nothing in this tree
needs more than one in flight per member** — the sequential model is
already what the code does, and the cap only makes it a guarantee.

**The guideline is available now, ahead of the apps that need it:**

> **Andy:** *"I do think that this should take priority to prove. because
> it will simplify the game later, give us UI guidelines etc."*

**One request at a time is what an app may assume.** An app written to
that needs no retrofit. One written without it does, and the number of
them only grows.
